BEGIN TRANSACTION;
DELETE FROM mg_meta WHERE schema_version = 6;
INSERT INTO mg_meta (schema_version) VALUES (7);

ALTER TABLE mg_raw_results
   ADD COLUMN detection_product TEXT,
   ADD COLUMN detection_method TEXT,
   ADD COLUMN detection_oid TEXT;

CREATE OR REPLACE VIEW mg_view_results AS
SELECT
    mg_raw_results.id,
